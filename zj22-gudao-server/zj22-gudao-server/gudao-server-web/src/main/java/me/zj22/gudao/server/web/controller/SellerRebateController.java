...
//        return new ModelAndView("common/success", map);
    ...
    public Integer freeze(String[] pks){
        try {
            for(int j=0; j<pks.length; j++){
                Rebate rebate = rebateService.selectByPrimaryKey(Integer.valueOf(pks[j]));
                rebate.setAvailable((byte)0);//冻结
                rebateService.freeze(rebate);
            }
            return 1;
    ...
    public Integer nofreeze(String[] pks){
        try {
            for(int j=0; j<pks.length; j++){
                Rebate rebate = rebateService.selectByPrimaryKey(Integer.valueOf(pks[j]));
                rebate.setAvailable((byte)1);//解冻
                rebateService.nofreeze(rebate);
            }
            return 1;
