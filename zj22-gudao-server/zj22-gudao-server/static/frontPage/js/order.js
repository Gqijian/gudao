...
*/
  ...
  queryURl(Base.orderAllUrl,function (data) {
       ...
       for (var i=0;i<data.length;i++){
          var state= checkTime(data[i].createTime)?'orderInfo.html?orderId='+data[i].orderId:'javascript:false';
           var str1="";
           str+='<a href="'+state+'" <figure>';
           str+='<div class="orderHeader">';
           str+= '<div class="fl"><div>店铺：酱豆</div><div>订单编号：<span class="orderId">'+data[i].orderId+'</span></div></div>';
           str+='<div class="fr"> 订单状态：';
           if(state != 'javascript:false'){//如果为true 则 显示时间
               str+='<span class="orderState state">交易关闭</span>';
               str+='<span class="orderTime"><span>30</span><span>:</span><span>00</span><span></span></span>';
           ...
           for(var j = 0;j<data[i].orderDetailList.length;j++){
               str1 += '<ul class="clearfloat">';
               str1 += '<li class="li1"><div><img src="'+data[i].orderDetailList[j].productIconOne+'" alt=""style="width: 60px;height: 60px"></div></li>';
               str1 += '<li class="li2"><span>'+data[i].orderDetailList[j].productName+'</span></li>';
               str1 += '<li class="li3"><span class="num">×'+data[i].orderDetailList[j].productQuantity+'</span><br/>￥<span style="color: red" >'+data[i].orderDetailList[j].price+'</span></li>';
               str1 += '</ul>';
           }
           str += str1;
           str += '<div>总价：<span class="allPrice">'+data[i].orderAmount+'</span></div>';
           str += '</div></figure>';
       }
       $(".container").html(str);
